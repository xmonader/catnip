...
fn do_tcp_establish_connection(port: u16) {
    ...
    let alice = thread::spawn(move || {
        ...
        let qd = FileDescriptor::from(unsafe { r.qr_value.ares.qd } as usize);
...
fn do_tcp_bad_bind(port: u16) {
    ...
    let port = ip::Port::try_from(port).unwrap();
    let local = ipv4::Endpoint::new(ALICE_IPV4, port);
    let e = libos.bind(FileDescriptor::from(0), local).unwrap_err();
...
fn do_tcp_bad_listen(port: u16) {
    ...
    // Invalid file descriptor.
    let e = libos.listen(FileDescriptor::from(0), 8).unwrap_err();
    assert_eq!(e, (Fail::BadFileDescriptor {}));
...
fn do_tcp_bad_accept() {
    ...
    // Invalid file descriptor.
    let e = libos.accept(FileDescriptor::from(0)).unwrap_err();
    assert_eq!(e, (Fail::BadFileDescriptor {}));
}
...
fn do_tcp_bad_connect(port: u16) {
    ...
    let bob = thread::spawn(move || {
        ...
        println!("BAD FD");
        // Bad file descriptor.
        let e = libos.connect(FileDescriptor::from(0), remote).unwrap_err();
        assert_eq!(e, (Fail::BadFileDescriptor {}));
